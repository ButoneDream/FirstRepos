...
import com.example.helpoldadmin.service.VolService;
...
//    @Qualifier("Blood_sugarServiceImpl")
    ...
    //添加数据
    @PostMapping("/add")
    public Result add(@RequestBody Vol bs) {
    ...
    //修改数据
    @PostMapping("/update")
    public Result update(@RequestBody Vol bs) {
        Result result = new Result();
        try {
            volService.update(bs);
            result.setMsg("修改成功!");
        } catch (Exception e) {
            result.status = false;
    ...
    public Map<String, Object> findByPage(Integer nowPage, Integer rows) {
        ...
        List<Vol> vol = volService.findByPage(nowPage, rows);
        Long totals = volService.findTotals();
        result.put("vol", vol);
