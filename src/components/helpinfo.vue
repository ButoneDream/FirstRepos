<template>
    <div id="blood_sugar">
        ...
        <el-table :data="tableData.filter(data => !search || data.cont.toLowerCase().includes(search.toLowerCase()))"
            ...
            <el-table-column label="互助信息" width="200" prop="cont" />
            <el-table-column label="发布时间" width="200" prop="date" />
            <el-table-column label="备注" width="200" prop="note">
            </el-table-column>
            <el-table-column align="right">
                <template slot="header" slot-scope="scope">
                    <el-input v-model="search" size="mini" style="width: auto" placeholder="输入互助信息关键字搜索" />
                </template>
...
export default {
    ...
    methods: {
        ...
        handleDelete(index, row) {
            ...
            this.axios.get("http://localhost:8082/hp/deleteByID?id=" + row.id).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
        ...
        onSubmit() {
            ...
            this.axios.post("http://localhost:8082/hp/add", this.form).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
        ...
        onSubmit2() {
            ...
            this.axios.post("http://localhost:8082/hp/update", this.form).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
                        message: '恭喜你,' + res.data.msg,
        ...
        showAllUsers(currentPage, pageSize) {  //异步请求显示所有数据
            ...
            this.axios.get("http://localhost:8082/hp/findByPage?nowPage=" + currentPage + "&rows=" + pageSize).then(res => {
                this.tableData = res.data.hp;
