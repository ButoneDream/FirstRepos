<template>
    <div id="blood_sugar">
        ...
        <!--员工信息列表-->
        <el-table :data="tableData.filter(data => !search || data.note.toLowerCase().includes(search.toLowerCase()))"
            style="width: 100%" height="300">
            <el-table-column label="用户编号" width="200" prop="uid" />
            <el-table-column label="编号" width="200" prop="id" />
            <el-table-column label="血糖值" width="200" prop="value" />
            <el-table-column label="记录时间" width="200" prop="date" />
            <el-table-column label="备注" width="200" prop="note">
            </el-table-column>
            <el-table-column align="right">
                <template slot="header" slot-scope="scope">
                    <el-input v-model="search" size="mini" style="width: auto" placeholder="输入备注关键字搜索" />
                </template>
...
export default {
    ...
    methods: {
        ...
        handleDelete(index, row) {
            ...
            this.axios.get("http://localhost:8082/bs/deleteByID?id=" + row.id).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
        ...
        onSubmit() {
            ...
            this.axios.post("http://localhost:8082/bs/add", this.form).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
        ...
        onSubmit2() {
            ...
            this.axios.post("http://localhost:8082/bs/update", this.form).then(res => {
                //判断执行状态
                if (res.data.status) {
                    this.$message({
                        message: '恭喜你,' + res.data.msg,
        ...
        showAllUsers(currentPage, pageSize) {  //异步请求显示所有数据
            ...
            this.axios.get("http://localhost:8082/bs/findByPage?nowPage=" + currentPage + "&rows=" + pageSize).then(res => {
                this.tableData = res.data.bs;
